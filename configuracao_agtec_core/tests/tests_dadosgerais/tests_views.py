...
from configuracao_agtec_core.models import DadosGerais
from configuracao_agtec_core.views import (
...
class TestDadosGeraisViews:
    ...
    def init(self, db):
        ...
        self.user = User.objects.create_superuser(username="teste", email="teste@email.com.br", password="asdf@1234")
        self.dadosgerais = baker.make(DadosGerais)

    def test_dadosgerais_list(self, init):
        """Teste para a view list."""
        url = reverse("configuracao_agtec_core:dadosgerais-list")
    ...
    def test_dadosgerais_detail(self, init):
        ...
        url = reverse("configuracao_agtec_core:dadosgerais-detail", args={self.dadosgerais.pk})
        request = self.factory.get(url)
        request.user = self.user
    ...
    def test_dadosgerais_create(self, init):
        ...
        url = reverse("configuracao_agtec_core:dadosgerais-create")
        request = self.factory.get(url)
        request.user = self.user
    ...
    def test_dadosgerais_update(self, init):
        ...
        url = reverse("configuracao_agtec_core:dadosgerais-update", args={self.dadosgerais.pk})
    ...
    def test_dadosgerais_delete(self, init):
        ...
        url = reverse("configuracao_agtec_core:dadosgerais-delete", args={self.dadosgerais.pk})
    ...
    def test_dadosgerais_queryset_superuser_status(self, init, client):
        ...
        request = self.factory.get(reverse("configuracao_agtec_core:dadosgerais-list"))
        request.user = self.user
        response = DadosGeraisListView.as_view()(request)
        assert response.status_code == 200
    ...
    def test_configuracao_agtec_core_index(self, init):
        """Teste para a view index."""
        url = reverse("configuracao_agtec_core:configuracao_agtec_core-index")
        request = self.factory.get(url)
        request.user = self.user
        response = Configuracao_Agtec_CoreIndexTemplateView.as_view()(request)
        assert response.status_code == 200
