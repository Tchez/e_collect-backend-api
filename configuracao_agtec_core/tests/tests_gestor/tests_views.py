...
from configuracao_agtec_core.models import Gestor
...
class TestGestorViews:
    ...
    def test_gestor_list(self, init):
        ...
        url = reverse("configuracao_agtec_core:gestor-list")
    ...
    def test_gestor_detail(self, init):
        ...
        url = reverse("configuracao_agtec_core:gestor-detail", args={self.gestor.pk})
        request = self.factory.get(url)
        request.user = self.user
    ...
    def test_gestor_create(self, init):
        ...
        url = reverse("configuracao_agtec_core:gestor-create")
        request = self.factory.get(url)
        request.user = self.user
    ...
    def test_gestor_update(self, init):
        ...
        url = reverse("configuracao_agtec_core:gestor-update", args={self.gestor.pk})
    ...
    def test_gestor_delete(self, init):
        ...
        url = reverse("configuracao_agtec_core:gestor-delete", args={self.gestor.pk})
    ...
    def test_gestor_queryset_superuser_status(self, init, client):
        ...
        request = self.factory.get(reverse("configuracao_agtec_core:gestor-list"))
        request.user = self.user
        response = GestorListView.as_view()(request)
        assert response.status_code == 200
