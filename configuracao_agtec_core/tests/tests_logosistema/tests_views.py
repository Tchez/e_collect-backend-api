...
from configuracao_agtec_core.models import LogoSistema
...
class TestLogoSistemaViews:
    ...
    def test_logosistema_list(self, init):
        ...
        url = reverse("configuracao_agtec_core:logosistema-list")
    ...
    def test_logosistema_detail(self, init):
        ...
        url = reverse("configuracao_agtec_core:logosistema-detail", args={self.logosistema.pk})
        request = self.factory.get(url)
        request.user = self.user
    ...
    def test_logosistema_create(self, init):
        ...
        url = reverse("configuracao_agtec_core:logosistema-create")
        request = self.factory.get(url)
        request.user = self.user
    ...
    def test_logosistema_update(self, init):
        ...
        url = reverse("configuracao_agtec_core:logosistema-update", args={self.logosistema.pk})
    ...
    def test_logosistema_delete(self, init):
        ...
        url = reverse("configuracao_agtec_core:logosistema-delete", args={self.logosistema.pk})
    ...
    def test_logosistema_queryset_superuser_status(self, init, client):
        ...
        request = self.factory.get(reverse("configuracao_agtec_core:logosistema-list"))
        request.user = self.user
        response = LogoSistemaListView.as_view()(request)
        assert response.status_code == 200
