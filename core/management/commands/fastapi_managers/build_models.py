        ...
        try:
            ...
            for related_field in iter(related_fields):
                ...
                if related_item.get("django_type") == "ManyToManyField":
                    ...
                    many_to_many += f"{table} = sa.Table('{table}', Base.metadata,"
                    ...
                    if _model_name == _related_model_name and _app_name == _related_model_app:
                        many_to_many += (
                            f"sa.Column('from_{_model_name}_id', ForeignKey('{_app_name}_{_model_name}.id')),"
                        )
                        many_to_many += f"sa.Column('to_{_related_model_name}_id', ForeignKey('{_related_model_app}_{_related_model_name}.id')))\n"
                    else:
                        many_to_many += f"sa.Column('{_model_name}_id', ForeignKey('{_app_name}_{_model_name}.id')),"
                        many_to_many += f"sa.Column('{_related_model_name}_id', ForeignKey('{_related_model_app}_{_related_model_name}.id')))\n"

                    result += f'\t{related_item.get("name")} = relationship("{__model.object_name}", secondary={table}, lazy="joined")\n'
