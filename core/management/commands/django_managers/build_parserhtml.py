...
class ParserHTMLBuild:
    def __init__(self, command, apps):
        ...
        self.template_update = Path(f"{self.templates_dir}/{self.model_lower}_update.html")
        self.template_create = Path(f"{self.templates_dir}/{self.model_lower}_create.html")
    ...
    def render_boolean_field_switch(self, field_name):
        ...
        """
        try:
            tag_result = "{{% if form.{field_name} in form.visible_fields %}}".format(
                field_name=field_name
            )
            tag_result += "<div class='col-md-6 mb-3'>"
            tag_result += "<div class='br-switch icon top mr-5'>"
            tag_result += "{{{{ form.{field_name} }}}}".format(field_name=field_name)
            tag_result += "{{{{ form.{field_name}.label_tag }}}}".format(
                field_name=field_name
            )
            tag_result += "<div class='invalid-feedback'>Campo Requerido.</div>"
            tag_result += "{{% if form.{field_name}.errors %}}".format(
                field_name=field_name
            )
            tag_result += "{{{{ form.{field_name}.errors }}}}".format(
                field_name=field_name
            )
            tag_result += "{% endif %}"
            tag_result += "</div></div>"
    ...
    def render_input(self, field):
        ...
        try:
            ...
            if iten["tipo"] in INPUT_TYPES:
                # Verificando se foi setado na constantes BOOLEAN_FIELD_IS_SWITCH
                if (
                    settings.BOOLEAN_FIELD_IS_SWITCH is True
                    and iten["tipo"] == "BooleanField"
                ):
                    return self.render_boolean_field_switch(iten["name"])
                ...
                if iten.get("tipo") in ("ForeignKey", "OneToOneField"):
                    ...
                    if hasattr(_model._meta, "fk_fields_modal") is True:
                        if iten["name"] in _model._meta.fk_fields_modal:
                            ...
                            _foreign_key_field += (
                                '"#form{}Modal">+</button>{{% endif %}}'.format(
    ...
    def manage_list_template(self, model):
        try:
            ...
            list_template = Path(f"{self.model_template_path}/{self.model_lower}_list.html")
